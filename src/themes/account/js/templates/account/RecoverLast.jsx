...
const RecoverLast = () => {
	...
	useEffect(() => {
		...
		const handleSubmit = (e) => {
			...
			* Check Code
			*/
			if(codeRef.current.value.length <= 21)
	            codeRef.current.classList.add('error');
	        else
	            codeRef.current.classList.remove('error');


	        /*
			* Check password
			*/
			if(pwdRef.current.value !== pwdConfirmRef.current.value || !window.passwordCheck(pwdRef.current.value)){
				pwdRef.current.classList.add('error');
				pwdConfirmRef.current.classList.add('error');
			} else{
				pwdRef.current.classList.remove('error');
				pwdConfirmRef.current.classList.remove('error');
			}
			...
			*/
			...
			formData.append('action', 'recover-last-step');
			formData.append('email', emailRef.current.value);
			formData.append('code', codeRef.current.value);
			formData.append('password', pwdRef.current.value);
			formData.append('password_confirmation', pwdConfirmRef.current.value);
						...
						<form ref={formRef} action="" method="POST">
							...
							<div className="fields">
								<div className="field">
									...
									<input ref={emailRef} type="email" id="email" autoComplete="email" required />
								</div>
								<div className="field">
									<label htmlFor="code">Code</label>
									<input ref={codeRef} type="text" id="code" required />
								</div>
								<div className="field">
									<label htmlFor="password">Mot de passe</label>
									<input ref={pwdRef} type="password" id="password" autoComplete="new-password" required />
								</div>
								<div className="field">
									<label htmlFor="confirm-password">Confirmer votre mot de passe</label>
									<input ref={pwdConfirmRef} type="password" id="confirm-password" autoComplete="new-password" required />
								</div>
							</div>
